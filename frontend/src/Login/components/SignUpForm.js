...
const SignUpForm = ({ showSignUp, setShowSignUp }) => {
  ...
  const checkValidNameHandler = () => {
    if ((firstName && !firstName.match(/^[a-zA-Z]+$/)) || (lastName && !lastName.match(/^[a-zA-Z]+$/))) {
      return <Alert variant="danger">Please enter an alphabetic name only!</Alert>;
    }
    return null;
  };
  ...
  const checkStrongPassword = () => {
    if (password && password.length < 8) {
      return <Alert variant="danger">Your password must be at least 8 characters long!</Alert>;
    }
    return null;
  };
  ...
  return (
    <Modal show={showSignUp} onHide={() => setShowSignUp(false)}>
      ...
      <Modal.Body>
        ...
        {checkStrongPassword()}
        <Form className="card p-3 bg-light" id="signup-form" onSubmit={signup}>
